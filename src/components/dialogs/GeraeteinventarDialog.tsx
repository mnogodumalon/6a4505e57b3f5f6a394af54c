...
import type { Geraeteinventar, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Geraeteinventar';
...
interface GeraeteinventarDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Geraeteinventar['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Geraeteinventar['fields'], 'kategorie' | 'zustand'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['geraeteinventar']?.['kategorie'] ?? [],
  zustand: LOOKUP_OPTIONS['geraeteinventar']?.['zustand'] ?? [],
};
...
export function GeraeteinventarDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: GeraeteinventarDialogProps) {
  const [fields, setFields] = useState<Partial<Geraeteinventar['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Geraeteinventar['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'geraeteinventar');
      await onSubmit(clean as Geraeteinventar['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Bezeichnung *\n  "inventarnummer": string | null, // Inventarnummer *\n  "kategorie": LookupValue | null, // Kategorie * (select one key: "it_hardware" | "software" | "maschinen" | "fahrzeuge" | "bueroausstattung" | "netzwerk_kommunikation" | "sonstiges") mapping: it_hardware=IT-Hardware, software=Software, maschinen=Maschinen, fahrzeuge=Fahrzeuge, bueroausstattung=Büroausstattung, netzwerk_kommunikation=Netzwerk & Kommunikation, sonstiges=Sonstiges\n  "zustand": LookupValue | null, // Zustand * (select one key: "neu" | "gut" | "in_betrieb" | "wartungsbedarf" | "defekt" | "ausgemustert") mapping: neu=Neu, gut=Gut, in_betrieb=In Betrieb, wartungsbedarf=Wartungsbedarf, defekt=Defekt, ausgemustert=Ausgemustert\n  "anschaffungswert": number | null, // Anschaffungswert (€)\n  "anschaffungsdatum": string | null, // YYYY-MM-DD\n  "standort": string | null, // Standort *\n  "verantwortlich_vorname": string | null, // Verantwortliche Person – Vorname\n  "verantwortlich_nachname": string | null, // Verantwortliche Person – Nachname\n  "hersteller": string | null, // Hersteller\n  "modell": string | null, // Modell / Typenbezeichnung\n  "seriennummer": string | null, // Seriennummer\n  "garantie_bis": string | null, // YYYY-MM-DD\n  "bemerkungen": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Geraeteinventar['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Geräteinventar bearbeiten' : 'Geräteinventar hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'zustand': (
      <div key="zustand" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="zustand" className="max-sm:h-11"><SelectValue placeholder="Aktueller Zustand des Geräts" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
    ...
    'anschaffungswert': (
      <div key="anschaffungswert" className="space-y-1.5">
        <Label htmlFor="anschaffungswert">Anschaffungswert (€)</Label>
        <Input
          id="anschaffungswert"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'anschaffungswert')}
          placeholder="z. B. 2500,00"
          value={fields.anschaffungswert !== undefined ? fields.anschaffungswert : (computedValues['anschaffungswert'] ?? '')}
          onChange={e => setFields(f => ({ ...f, anschaffungswert: clampNumberValue(formEnhancements, 'anschaffungswert', e.target.value) }))}
        />
      </div>
    ),
    'anschaffungsdatum': (
    ...
    'garantie_bis': (
      <div key="garantie_bis" className="space-y-1.5">
        <Label htmlFor="garantie_bis">Garantie gültig bis</Label>
        <DatePicker
          id="garantie_bis"
          placeholder="Garantie endet wann?"
          mode="date"
    ...
    'bemerkungen': (
      <div key="bemerkungen" className="space-y-1.5">
        ...
        <Textarea
          id="bemerkungen"
          placeholder="Besonderheiten, Wartungshistorie, Zubehör..."
    ...
    'foto': (
      <div key="foto" className="space-y-1.5">
        <Label htmlFor="foto">Foto / Dokument</Label>
        {fields.foto ? (
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Bezeichnung *", "inventarnummer": "Inventarnummer *", "kategorie": "Kategorie *", "zustand": "Zustand *", "anschaffungswert": "Anschaffungswert (€)", "anschaffungsdatum": "Anschaffungsdatum", "standort": "Standort *", "verantwortlich_vorname": "Verantwortliche Person – Vorname", "verantwortlich_nachname": "Verantwortliche Person – Nachname", "hersteller": "Hersteller", "modell": "Modell / Typenbezeichnung", "seriennummer": "Seriennummer", "garantie_bis": "Garantie gültig bis", "bemerkungen": "Bemerkungen", "foto": "Foto / Dokument"};
  const CURRENCY_KEYS = new Set<string>(["anschaffungswert"]);
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.GERAETEINVENTAR} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
