...
import type { Geraeteinventar, CreateGeraeteinventar } from '@/types/app';
...
export class LivingAppsService {
  // --- GERAETEINVENTAR ---
  static async getGeraeteinventar(): Promise<Geraeteinventar[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERAETEINVENTAR}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Geraeteinventar[];
    return enrichLookupFields(records, 'geraeteinventar');
  }
  static async getGeraeteinventarEntry(id: string): Promise<Geraeteinventar | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERAETEINVENTAR}/records/${id}`);
    const record = { record_id: data.id, ...data } as Geraeteinventar;
    return enrichLookupFields([record], 'geraeteinventar')[0];
  }
  static async createGeraeteinventarEntry(fields: CreateGeraeteinventar) {
    return callApi('POST', `/apps/${APP_IDS.GERAETEINVENTAR}/records`, { fields: cleanFieldsForApi(fields as any, 'geraeteinventar') });
  }
  static async updateGeraeteinventarEntry(id: string, fields: Partial<CreateGeraeteinventar>) {
    return callApi('PATCH', `/apps/${APP_IDS.GERAETEINVENTAR}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'geraeteinventar') });
  }
  static async deleteGeraeteinventarEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GERAETEINVENTAR}/records/${id}`);
  }
