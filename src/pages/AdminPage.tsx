...
import { GeraeteinventarViewDialog } from '@/components/dialogs/GeraeteinventarViewDialog';
...
const GERAETEINVENTAR_FIELDS = [
  { key: 'bezeichnung', label: 'Bezeichnung *', type: 'string/text' },
  { key: 'inventarnummer', label: 'Inventarnummer *', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie *', type: 'lookup/select', options: [{ key: 'it_hardware', label: 'IT-Hardware' }, { key: 'software', label: 'Software' }, { key: 'maschinen', label: 'Maschinen' }, { key: 'fahrzeuge', label: 'Fahrzeuge' }, { key: 'bueroausstattung', label: 'Büroausstattung' }, { key: 'netzwerk_kommunikation', label: 'Netzwerk & Kommunikation' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'zustand', label: 'Zustand *', type: 'lookup/select', options: [{ key: 'neu', label: 'Neu' }, { key: 'gut', label: 'Gut' }, { key: 'in_betrieb', label: 'In Betrieb' }, { key: 'wartungsbedarf', label: 'Wartungsbedarf' }, { key: 'defekt', label: 'Defekt' }, { key: 'ausgemustert', label: 'Ausgemustert' }] },
  { key: 'anschaffungswert', label: 'Anschaffungswert (€)', type: 'number' },
  { key: 'anschaffungsdatum', label: 'Anschaffungsdatum', type: 'date/date' },
  { key: 'standort', label: 'Standort *', type: 'string/text' },
  { key: 'verantwortlich_vorname', label: 'Verantwortliche Person – Vorname', type: 'string/text' },
  { key: 'verantwortlich_nachname', label: 'Verantwortliche Person – Nachname', type: 'string/text' },
  ...
  { key: 'garantie_bis', label: 'Garantie gültig bis', type: 'date/date' },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
  { key: 'foto', label: 'Foto / Dokument', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'geraeteinventar', label: 'Geräteinventar', pascal: 'Geraeteinventar' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('geraeteinventar');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'geraeteinventar': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'geraeteinventar': return (data as any).geraeteinventar as Geraeteinventar[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'geraeteinventar': return GERAETEINVENTAR_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'geraeteinventar': return {
        create: (fields: any) => LivingAppsService.createGeraeteinventarEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateGeraeteinventarEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteGeraeteinventarEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'geraeteinventar' || dialogState?.entity === 'geraeteinventar') && (
        <GeraeteinventarDialog
          open={createEntity === 'geraeteinventar' || dialogState?.entity === 'geraeteinventar'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'geraeteinventar' ? handleUpdate : (fields: any) => handleCreate('geraeteinventar', fields)}
          defaultValues={dialogState?.entity === 'geraeteinventar' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Geraeteinventar']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Geraeteinventar']}
        />
      )}
      {viewState?.entity === 'geraeteinventar' && (
        <GeraeteinventarViewDialog
          open={viewState?.entity === 'geraeteinventar'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'geraeteinventar', record: r }); }}
        />
      )}
