...
export default function GeraeteinventarPage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Geraeteinventar[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Geraeteinventar | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Geraeteinventar | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getGeraeteinventar());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Geraeteinventar['fields']) {
  ...
  async function handleUpdate(fields: Geraeteinventar['fields']) {
    ...
    await LivingAppsService.updateGeraeteinventarEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteGeraeteinventarEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Geräteinventar"
      subtitle={`${records.length} Geräteinventar im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Geräteinventar suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('inventarnummer')}>
                <span className="inline-flex items-center gap-1">
                  Inventarnummer *
                  {sortKey === 'inventarnummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anschaffungswert')}>
                <span className="inline-flex items-center gap-1">
                  Anschaffungswert (€)
                  {sortKey === 'anschaffungswert' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anschaffungsdatum')}>
                <span className="inline-flex items-center gap-1">
                  Anschaffungsdatum
                  {sortKey === 'anschaffungsdatum' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('verantwortlich_vorname')}>
                <span className="inline-flex items-center gap-1">
                  Verantwortliche Person – Vorname
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('verantwortlich_nachname')}>
                <span className="inline-flex items-center gap-1">
                  Verantwortliche Person – Nachname
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('garantie_bis')}>
                <span className="inline-flex items-center gap-1">
                  Garantie gültig bis
                  {sortKey === 'garantie_bis' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/geraeteinventar/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.bezeichnung ?? '—'}</TableCell>
                <TableCell>{record.fields.inventarnummer ?? '—'}</TableCell>
                ...
                <TableCell>{record.fields.anschaffungswert ?? '—'}</TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.anschaffungsdatum)}</TableCell>
                <TableCell>{record.fields.standort ?? '—'}</TableCell>
                <TableCell>{record.fields.verantwortlich_vorname ?? '—'}</TableCell>
                ...
                <TableCell className="text-muted-foreground">{formatDate(record.fields.garantie_bis)}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={16} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Geräteinventar. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
